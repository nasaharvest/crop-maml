...
class BaseEngineer(ABC):
    ...
    def process_filename(
        ...
        try:
            ...
            if include_extended_filenames:
                ...
                end_year, end_month, end_day = (end_list[0], end_list[1], end_list[2])
    ...
    @staticmethod
    def _find_central(da: xr.Dataset, overlap: pd.DataFrame) -> int:
        r"""
        Return the index of the most central row in `overlap`
        to `da`
        """
        # let's find the most central
        # point
        mid_x = math.ceil(len(da.x) / 2)
        mid_y = math.ceil(len(da.y) / 2)

        mid_lon, mid_lat = float(da.x[mid_x]), float(da.y[mid_y])

        # this is crude, but roughly calculates
        # the distance of the overlapping points from the
        # center of the tif
        with warnings.catch_warnings():
            # we get a SettingWithCopyWarning from pandas here, but
            # we are not setting on a view so this is safe to do
            warnings.simplefilter("ignore")
            overlap["distance"] = abs(overlap.lat - mid_lat) + abs(overlap.lon - mid_lon)
        return overlap["distance"].idxmin()
