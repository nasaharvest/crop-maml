...
from src.processors import SudanProcessor
from src.exporters import SudanSentinelExporter
...
@dataclass
class SudanDataInstance(BaseDataInstance):
    is_crop: int


class SudanEngineer(BaseEngineer):

    sentinel_dataset = SudanSentinelExporter.dataset
    dataset = SudanProcessor.dataset

    @staticmethod
    def read_labels(data_folder: Path) -> pd.DataFrame:
        togo = data_folder / "processed" / SudanProcessor.dataset / "data.geojson"
        assert togo.exists(), "Sudan processor must be run to load labels"
