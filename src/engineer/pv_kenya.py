...
class PVKenyaEngineer(BaseEngineer):
    ...
    def process_single_file(
        ...
        if len(overlap) == 0:
            ...
            if len(overlap) > 1:
                overlap_idx = self._find_central(da, overlap)
            else:
                overlap_idx = overlap.index[0]

            label_lat = overlap.loc[overlap_idx].lat
            label_lon = overlap.loc[overlap_idx].lon

            crop_type = overlap.loc[overlap_idx].crop_type
