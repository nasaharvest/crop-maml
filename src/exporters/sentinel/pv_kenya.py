...
class KenyaPVSentinelExporter(BaseSentinelExporter):
    ...
    def labels_to_bounding_boxes(
        ...
        for idx, row in tqdm(self.labels.iterrows()):
            ...
            try:
                harvest_date = datetime.strptime(row["harvest_da"], "%Y-%m-%dT%H:%M:%S").date()
            except ValueError:
                continue
            ...
            if end_month is not None:
                planting_date = datetime.strptime(row["planting_d"], "%Y-%m-%dT%H:%M:%S").date()
    ...
    def export_for_labels(
        ...
        r"""
        If start_month is None, then the harvest date is used.
        Otherwise, the most overlapping year which ends with
        end_month is used
        """
